...
> 강사 설명: ERD는 개발자뿐 아니라 기획자, 디자이너 등 다양한 직군이 데이터 구조를 공통으로 이해하기 위한 설계도이다. **데이터베이스 모델링은 실제 코드 작성 전에 가장 먼저 진행해야 한다.** 미리 ERD를 그려보면 중복 설계나 누락된 관계를 사전에 파악하고, 팀원 간 소통 비용도 줄일 수 있다.
...
> 강사 조언: 관통 프로젝트 시 페어 프로그래밍을 시작하기 전에, 서로 ERD를 먼저 그려보고 모델 관계를 합의한 후 개발을 시작하는 것을 권장한다.
